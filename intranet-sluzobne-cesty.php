	...
	<?php
		...
		if(isset($_POST['nazovp']) && isset($_FILES['subor'])){
			...
			if (move_uploaded_file($_FILES['subor']['tmp_name'], $uploadfile)) {
				...
				$sql = "INSERT INTO intranet (Kategoria , Prilohy , subor , stranka) VALUES('$kategoria','$pr','$file','sluzcesty') ";
				$conn->query($sql);
		...
		if(isset($_POST['idp'])){
			...
			//echo $uploadfile;
			unlink($uploadfile);
			$sql= "DELETE FROM intranet WHERE Prilohy = '$priloha1' AND stranka = 'sluzcesty'";
			$conn->query($sql);
			...
			if(isset($_POST[$ID]) && strlen($_POST[$ID]) >2 && isset($_POST['edit'])){
				$sql= "UPDATE intranet SET Kategoria='$kat' WHERE Kategoria = '$kategoria' AND stranka = 'sluzcesty'";
				$conn->query($sql);
				
			}else if(isset($_POST['del'])){
				define ('SITE_ROOT', realpath(dirname(__FILE__)));
				$sql= "SELECT * FROM intranet WHERE Kategoria = '$kategoria' AND stranka = 'sluzcesty'";
				...
				$sql= "DELETE FROM intranet WHERE Kategoria = '$kategoria' AND stranka = 'sluzcesty'";
				$conn->query($sql);
			}
		}else if(isset($_POST['newk']) && strlen($_POST['newk']) > 2){
					//echo aaa;
					$kateg = $_POST['newk'];
					$sql = "INSERT INTO intranet (Kategoria , stranka) VALUES('$kateg','sluzcesty')";
					$conn->query($sql);
        ...
        <div class="row">
            <div class="col-md-12 wow fadeIn">
                <table class="table table-bordered table-striped">
					...
					if($log == 1){
						$sql = "SELECT * FROM intranet WHERE stranka = 'sluzcesty' ORDER BY Kategoria , Prilohy";
						$result = $conn->query($sql);
						$kategoria = "";
						$tmp = 0;
						$Kid = 0;
